...
function NomenklaturaDetailContent() {
  ...
  const lastCreatedTemplateName = useMemo(() => {
    if (specTemplates.length === 0) return null;
    const [first, ...rest] = specTemplates;
    if (!first) return null;
    const last = rest.reduce<SpecTemplate>(
      (best, cur) =>
                      ...
                      >
                        <div className="relative">
                          <Combobox.Input
                            ...
                            placeholder={
                              lastCreatedTemplateName
                                ? `Например, ${lastCreatedTemplateName}`
                                : "Начните вводить…"
                            }
