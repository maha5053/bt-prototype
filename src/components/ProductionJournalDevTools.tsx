...
/**
 * Плавающая шестерёнка (как на карантине): сброс только списка заказов в ключе
 * «Производство», без шаблонов и реестра полей.
 */
export function ProductionJournalDevTools() {
  const [open, setOpen] = useState(false);

  const clearOrdersOnly = () => {
    try {
        ...
        >
          ...
          >
            ...
            <p className="mb-4 text-sm text-slate-600">
              Будут удалены только <strong>заказы</strong> из ключа «{PRODUCTION_STORAGE_KEY}»
              (журнал на этой странице). Шаблоны процессов и реестр полей останутся без изменений.
              После подтверждения страница перезагрузится.
            </p>
