...
/**
 * Devtools для «Типы материала»: сброс только materialTypes в ключе «Производство»
 * (шаблоны, заказы и реестр полей не трогаем).
 */
export function MaterialTypesDevTools() {
  const [open, setOpen] = useState(false);

  const clearMaterialTypesOnly = () => {
    try {
        ...
        >
          ...
          >
            ...
            <p className="mb-4 text-sm text-slate-600">
              Будут сброшены только <strong>настройки типов материала</strong> в ключе «
              {PRODUCTION_STORAGE_KEY}». Шаблоны продуктов, журнал заказов и реестр полей
              останутся без изменений. После подтверждения страница перезагрузится и подтянутся
              стандартные типы из моков.
            </p>
