...
- **Эта сессия (@end-session):** Phase 3 закрыта (хранение, матбаланс регистрации, snapshot/runtime); в «Продукты» настройки перенесены во вкладки **Регистрация** (тип материала + матбаланс) и **Хранение** (чекбокс этапа и поля); на «Типы материала» — devtools-шестерёнка вместо «Восстановить типы» (сброс только `materialTypes` в `bio-production`). Следующий фокус — Phase 4 independent QC (`$gsd-plan-phase 4`). Коммит, push, deploy выполнены в конце сессии.
