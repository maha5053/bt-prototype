...
- **Брак (модалка + данные)**: этап брака (входной материал / производство / QC / логистика), вложения файлов/фото (data URL, лимиты 6×4 МБ), сохранение в заказе; в карточке `rejected` — этап и список вложений с превью картинок. Типы `ProductionRejectionPhase`, `ProductionRejectionAttachment` в `productionData.ts`; `rejectOrder` и dev-ветка `setOrderStatus` → rejected обновлены в `ProductionContext.tsx`.
- **Журнал производства**: под бейджем «В работе» выводится название текущего этапа (`StatusBadge.currentStageLabel`).
- **Степпер / шаги**: бейдж статуса только для **«В работе»** (этап: `in_progress` и не `deferred`; шаг: только синтетический `in_progress`). Исправлена ошибка, когда на неактивном этапе шаги с `pending` подписывались как «в работе».
- **Производство, многошаговый этап**: в сайдбаре и заголовке формы — «Шаг N из M: …», строка «Завершено X из M» (в многошаговом этапе — под «Шаги», в одношаговом — под заголовком).
- **КК**: после показателей поле **«Комментарий»** (`qcComment`, `multiline`).
- **Доступность**: ссылка на файл СОП в заголовке секции с `tabIndex={-1}` (не в порядке Tab).
- **localStorage**: новые поля шаблона/заказа могут не появиться у старых сохранённых заказов до сброса `bio-production` — при необходимости dev-tools в журнале.
