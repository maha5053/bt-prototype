...
- Роут вне layout (как списания): `path: "proizvodstvo/:orderId/print"` → [`src/pages/ProductionReleasePrintPage.tsx`](src/pages/ProductionReleasePrintPage.tsx), `ProductionProvider`, авто-`window.print()`, класс `print-container`.
- Данные: [`src/lib/productionReleaseAct.ts`](src/lib/productionReleaseAct.ts) — `getReleaseIssueConfirmSummary`, `isReleaseStageCompleted`, `getRegistrationPatientFields` (общие с модалкой подтверждения выдачи в [`ProductionPages.tsx`](src/pages/ProductionPages.tsx)).
- Меню **⋯** в шапке заказа: пункты «Печать акта выдачи» и «Забраковать» **всегда видны**; печать активна после завершения этапа `release`, забраковать — только пока заказ `in_progress`. Открытие печати: `window.open(\`${import.meta.env.BASE_URL}proizvodstvo/${orderId}/print\`, ...)`.
...
Полный план модуля: `.cursor/plans/production_process_module_e6aef1f7.plan.md` (или копия в корне воркспейса при необходимости). Отдельный файл `tasks/production-ux-plan.md` **удалён** — актуальный бэклог UX по производству закрыт в прототипе; дальнейшие задачи — из плана модуля / постановок продукта.
...
- **Типы полей**: number, text, checkbox, select, date, section_header; для `text` опционально **`multiline`** (textarea в UI). Поддержка вычисляемых полей (`computeRule`), ссылок на другие этапы (`refStageIndex` + `refFieldId`), ссылок на СОП (`sopRef`).
...
- **UI “Производство”**:
  - Журнал `/proizvodstvo`: таблица заказов + создание заказа (модалка выбора **продукта**); **пагинация** 15 записей на страницу (как у списаний); **фильтры** в модалке (статус, продукт/шаблон, текущий этап); **поиск только по ФИО и № ИБ** из этапа регистрации (`fio` / `ib` в данных шага); **сортировка** кликом по заголовкам: номер заказа, продукт, **дата регистрации**; колонки **«Статус»** и **«Текущий этап»** в таком порядке; для **«Брак»** и **«Завершён»** колонка текущего этапа пустая, под бейджем статуса название этапа не дублируется; отображение этапов **без префикса «N. »** в пиллах/заголовках (`formatStageLabel` + правки моков шаблона). Статус «Брак» — красный бейдж + причина отдельной строкой; dev-tools (шестерёнка) для сброса `bio-production`
  - Заказ `/proizvodstvo/:orderId`: stepper этапов (`StageStepper`), формы этапов/шагов, последовательность шагов (нельзя редактировать шаг 2 пока не завершён шаг 1), для этапов с 1 шагом скрыт сайдбар «Шаги»; после завершения этапа — прокрутка `main` к началу (`scrollProductionOrderMainToTop`)
  - Меню действий **⋯** — в одной строке с метаинформацией заказа (не у степпера); см. раздел «Печать акта выдачи»
  - У шага в шаблоне опциональны `sopRef` / `sopFileName` — ссылка на моковый PDF рядом с заголовком шага (`SopAttachmentLink`)
  - Ref-поля выдачи: подсветка одного инпута (`FieldInput` + `tone="crossStageRef"`), пилл «из этапа»
  - Брак: пункт «Забраковать» в меню ⋯, модалка с обязательной причиной, `rejected` — read-only с причиной/кем/когда
...
## Последняя сессия (ориентир по производству)

- **`StageStepper`**: кружки и сегменты линии как раньше; выбранный этап без отдельного фона карточки; кольцо выбора по цвету состояния этапа (в т.ч. ожидание — **slate**, не янтарный); ховер — лёгкое увеличение кружка, смена цвета названия этапа по состоянию; фокус-кольцо на кружке.
- **Выдача**: **`ReleaseIssueConfirmModal`**; сводка и печать через **`src/lib/productionReleaseAct.ts`** (`getReleaseIssueConfirmSummary`, `isReleaseStageCompleted`).
- **Печать акта выдачи**: отдельный роут и [`ProductionReleasePrintPage.tsx`](src/pages/ProductionReleasePrintPage.tsx); пункт в меню ⋯ (см. выше).
- **Меню ⋯**: оба пункта всегда в списке, с `disabled` и подсказками до/после завершения выдачи; **`cursor-pointer`** на пунктах и кнопке ⋯.
- **Формы**: чеклист производства, ref-поля с **`tone="crossStageRef"`** (один контур инпута); СОП у шага — `StepTemplate.sopRef` / `sopFileName`.
- **`productionData.ts`**: **`PRODUCTION_REJECTION_PHASE_LABELS`**; шаг производства с примером СОП в моке.
- **localStorage**: при аномалиях — сброс **`bio-production`** через dev-tools в журнале.

## Сессия end-task (2026-04-18)

- Печать акта выдачи: `ProductionReleasePrintPage.tsx`, роут `proizvodstvo/:orderId/print` в `App.tsx`, данные в `lib/productionReleaseAct.ts` (сводка выдачи, `isReleaseStageCompleted`, ФИО/ИБ из регистрации).
- Меню ⋯ в шапке заказа: «Печать акта выдачи» и «Забраковать» всегда в списке; активность по завершению этапа `release` и статусу заказа; курсор `pointer` на пунктах.
- Формы: ref-поля — один стиль инпута (`FieldInput` + `tone="crossStageRef"`); степпер — серое кольцо у выбранного этапа «Ожидается».
- Документация: `agents.md` обновлён; файл `tasks/production-ux-plan.md` удалён из воркспейса (не в git `bt-prototype/`).
